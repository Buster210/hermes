...
# Pin to a specific commit/tag for reproducible, cacheable builds
...
COPY --chown=hermes:hermes .env .
COPY --chown=hermes:hermes entrypoint.sh .
RUN chmod +x entrypoint.sh

# Check that the hermes gateway process is alive, not just the liveness HTTP server
